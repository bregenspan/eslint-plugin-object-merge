...
module.exports = {
  ...
  create (context) {
    ...
    return {
      CallExpression (node) {
        if (!options.functionNames.includes(node.callee.name) || !node.arguments.length) {
          return;
        }

        const scope = context.getScope();

        if (isImportedFromPackage(scope, options.functionNames, options.packageNames) && containsIdentifier(node.arguments[0])) {
          context.report(node.arguments[0],
            `This use of '${node.callee.name}()' could cause unexpected side effects. Insert an empty object as first argument.`
          );
        }
      }
